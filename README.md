...
泳道（Swimlane）将活动按角色分组，每个角色一条横向泳道，类似 Visio 的跨职能流程图。
...
#### `add_swimlane(x, y, w, h, label, color='#E8F0FE', header_w=40)`
...
| `x`, `y` | int | — | 左上角绝对坐标 |
| `w`, `h` | int | — | 泳道宽高 |
| `label` | str | — | 角色名（显示在左侧头） |
| `color` | str | `'#E8F0FE'` | 填充色 |
| `header_w` | int | `40` | 左侧角色头宽度 |

```python
lane = ly.add_swimlane(30, 30, 900, 110, '商家 (Merchant)', color='#FFF3CD')
```

#### `add_label_in_lane(lane_id, lane_x, lane_y, x_rel, y_rel, w, h, text, font_size=11, bold=False, color='#333333')`

在泳道内添加**无边框文字标签**。标签没有描边/填充色，以父泳道为视觉容器。`lane_x/lane_y` 是泳道的绝对坐标，用于计算标签的绝对位置（供路由算法使用）；`x_rel/y_rel` 是相对于泳道的偏移。

```python
step = ly.add_label_in_lane(lane, abs_x, abs_y, 80, 35, 140, 36,
                             '提交申请', bold=True, color='#8B6914')
```

### 关键规则

1. **标签无边框** — `add_label_in_lane` 生成的 `mxCell` 使用 `style="text;..."`（纯文字，无 `strokeColor`/`fillColor`），泳道容器提供视觉边界
